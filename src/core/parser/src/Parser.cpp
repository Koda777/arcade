...
Arcade::Parser::Parser(std::string defaultPath)
{
...
bool Arcade::Parser::reload()
{
    ...
    if (_libs.empty()) {
        throw Arcade::NoLibException("No .so files found in the lib folder");
    }
...
std::vector<std::string> Arcade::Parser::getGames()
{
    return _games;
}

std::vector<std::string> Arcade::Parser::getGraphics()
{
    return _graphics;
}
